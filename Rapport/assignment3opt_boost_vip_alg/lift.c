...
lift_type lift_create(void) 
{
    ...
    pthread_cond_init(&lift->cond_enter_exit,NULL);
    pthread_cond_init(&lift->cond_new_floor,NULL);
    for(i = 0; i < N_FLOORS; i++){
        pthread_cond_init(&lift->cond_floor[i],NULL);
    }
    sem_init(&lift->sem, 0, 0);
    sem_init(&lift->sem_done, 0, 0);
...
/* Check if lift is done evacuating */
static int lift_done_evacuating(lift_type lift){
    ...
    return evacuation_complete;

}

/* Check if lift is done boarding */
static int lift_done_boarding(lift_type lift){
    ...
    //int lift_full = (n_passengers_in_lift(lift) == MAX_N_PASSENGERS); // Should we check this first to speed up? ////////////////////////TODO: We did it!
    return (boarding_complete || lift_full);
...
{
    pthread_mutex_lock(&lift->mutex);
    //pthread_cond_broadcast(&lift->change); // broadcast cond_new_floor ////////////
    pthread_cond_broadcast(&lift->cond_new_floor);
    while(!lift_done_evacuating(lift)){
    	//pthread_cond_wait(&lift->change,&lift->mutex); // wait cond_enter_exit
	pthread_cond_wait(&lift->cond_enter_exit,&lift->mutex);
    }

    //pthread_cond_broadcast(&lift->change); // broadcast cond_floor[lift->floor??]
    pthread_cond_broadcast(&lift->cond_floor[lift->floor]);
    while(!lift_done_boarding(lift)){
    	//pthread_cond_wait(&lift->change,&lift->mutex); // wait cond_enter_exit
	pthread_cond_wait(&lift->cond_enter_exit,&lift->mutex);
    }
...
static int passenger_wait_for_lift(lift_type lift, int wait_floor, int id)
{
    int i;
    int vip_found = 0;
    for(i = 0; i < MAX_N_PERSONS; i++){
	if((lift->persons_to_enter[wait_floor][i].id < N_VIP) &&
		(lift->persons_to_enter[wait_floor][i].id > -1)){
	    vip_found = 1;
	}
    }
    if(vip_found && id>=N_VIP){
        return 1;
    }
    int waiting_ready =
        /* the lift is not moving */ 
...
void lift_travel(lift_type lift, int id, int from_floor, int to_floor)
{
    ...
    //printf("hre before\n");
    while(passenger_wait_for_lift(lift, from_floor, id)){ 
        //pthread_cond_wait(&lift->change,&lift->mutex); // wait for cond_floor[from_floor] /////////////////////////////////////////////////
    ...
    if(id < N_VIP){
	pthread_cond_broadcast(&lift->cond_floor[from_floor]);
    }
    //pthread_cond_broadcast(&lift->change); // broadcast cond_enter_exit /////////////////////////////
    pthread_cond_broadcast(&lift->cond_enter_exit);
    while(passenger_wait_for_floor(lift, to_floor)){
        //pthread_cond_wait(&lift->change,&lift->mutex); // wait for cond_new_floor //////////////////////////////////
